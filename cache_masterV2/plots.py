...
def create_cache_hitrate_algorithm_gnu(minhit, maxhit, min_cache, max_cache, algorithm):
    folder = results +algorithm
    
    file_name_gnu = graphics + 'cache_hitrate_'+algorithm+'.gnu'
    file_name_png = graphics + 'cache_hitrate_'+algorithm+'.png'
    ...
    for fn in os.listdir(folder):
        ...
        if p == len (os.listdir(folder)):
            notfirst = notfirst + "\""+folder+'/'+fn+"\" using 2:3  title \""+title+"\" "
        else:
            notfirst = notfirst + "\""+folder+'/'+fn+"\" using 2:3  title \""+title+"\" , "
        p+=1
...
def plot_cache_vs_hitrate_algorithm(algorithm):
    file_name_gnu = "\""+graphics+"\""+'cache_hitrate_'+algorithm+'.gnu'
    system('gnuplot ' + file_name_gnu)
    remove(graphics+'cache_hitrate_'+algorithm+'.gnu')
